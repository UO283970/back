...
public class GetFollowingListUserTest {
    ...
    @Test
    public void PU137_GetFollowingListUser() {
        when(authenticatedUserIdProvider.getUserId()).thenReturn("userId1");

        String userId = "userId1";

        List<UserForProfile> authenticatedUserInfo = userService.getFollowingListUser(userId);

        assertThat(authenticatedUserInfo).isNotNull();
        assertThat(authenticatedUserInfo.isEmpty()).isFalse();
        assertThat(authenticatedUserInfo.size()).isEqualTo(2);
    }

    @Test
    public void PU138_GetFollowingListUser() {
        when(authenticatedUserIdProvider.getUserId()).thenReturn("userId1");
    ...
    @Test
    public void PU139_GetFollowingListUser() {
        when(authenticatedUserIdProvider.getUserId()).thenReturn("userId1");
    ...
    @Test
    public void PU140_GetFollowingListUser() {
        when(authenticatedUserIdProvider.getUserId()).thenReturn("userId1");
    ...
    @Test
    public void PU141_GetFollowingListUser() {
        String userId = "notAUserId";

        doThrow(new RuntimeException("Simulated Firestore failure"))
                .when(userService)
                .getFollowingListUser(userId);

        assertThrows(RuntimeException.class, () -> {
            userService.getFollowingListUser(userId);
        });
    }
}
